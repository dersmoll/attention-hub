...
export async function openManagerWindow(focus: "projects" | "todos" = "projects", projectId?: string) {
  const existing = await WebviewWindow.getByLabel(MANAGER_WINDOW_LABEL);
  if (existing) { await existing.setTitle(MANAGER_WINDOW_TITLE); await existing.show(); await existing.setFocus(); await emitTo(MANAGER_WINDOW_LABEL, MANAGER_FOCUS_EVENT, { focus, projectId }); return; }
