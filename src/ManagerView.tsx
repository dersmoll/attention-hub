...
type ManagerActionIconName = "archive" | "delete" | "down" | "edit" | "notes" | "restore" | "up";

function ManagerActionIcon({ name }: { name: ManagerActionIconName }) {
  if (name === "edit") {
      ...
      <section className="manager-items__section manager-items__personal">
        ...
        {personalCategories.map((category, categoryIndex) => <div className="manager-category" key={category.id}>
          <div className="manager-category__header">{editingCategoryId === category.id
            ...
            : <><p>{category.name}</p><span><button aria-label={`Move ${category.name} up`} disabled={categoryIndex === 0} onClick={() => void swapOrder("move_personal_category", "categoryId", personalCategories, categoryIndex, -1)} type="button"><ManagerActionIcon name="up"/></button><button aria-label={`Move ${category.name} down`} disabled={categoryIndex === personalCategories.length - 1} onClick={() => void swapOrder("move_personal_category", "categoryId", personalCategories, categoryIndex, 1)} type="button"><ManagerActionIcon name="down"/></button><button aria-label={`Rename ${category.name}`} onClick={() => { setEditingCategoryId(category.id); setCategoryName(category.name); }} type="button"><ManagerActionIcon name="edit"/></button><button aria-label={`Delete ${category.name}`} className="is-danger" onClick={() => void prepareCategoryDelete(category.id)} type="button"><ManagerActionIcon name="delete"/></button></span></>}
          </div>
      ...
      {!compact && managerSection === "all-todos" ? <>
        ...
        <header className="manager-detail__header" onPointerDown={compact ? (pointerEvent) => { if (!(pointerEvent.target as HTMLElement).closest("button, input, select, a")) void getCurrentWindow().startDragging(); } : undefined}>
          ...
          {!renaming && !compact && <div className="manager-owner-actions">
            <button aria-label="Move up" className="manager-icon-action" disabled={selectedOrderIndex <= 0} onClick={() => selected && void swapOrder(selected.kind === "project" ? "move_project" : "move_list", selected.kind === "project" ? "projectId" : "listId", selectedOrder, selectedOrderIndex, -1)} title="Move up" type="button"><ManagerActionIcon name="up"/></button>
            <button aria-label="Move down" className="manager-icon-action" disabled={selectedOrderIndex < 0 || selectedOrderIndex === selectedOrder.length - 1} onClick={() => selected && void swapOrder(selected.kind === "project" ? "move_project" : "move_list", selected.kind === "project" ? "projectId" : "listId", selectedOrder, selectedOrderIndex, 1)} title="Move down" type="button"><ManagerActionIcon name="down"/></button>
            <button aria-label="Rename" className="manager-icon-action" onClick={() => { setRenameValue(owner.name); setRenaming(true); }} title="Rename" type="button"><ManagerActionIcon name="edit"/></button>
            {project && <button aria-label={project.archivedAt ? "Restore project" : "Archive project"} className="manager-icon-action" onClick={() => void archiveProject()} title={project.archivedAt ? "Restore project" : "Archive project"} type="button"><ManagerActionIcon name={project.archivedAt ? "restore" : "archive"}/></button>}
            <button aria-label="Delete" className="manager-icon-action is-danger" onClick={() => void prepareDelete()} title="Delete" type="button"><ManagerActionIcon name="delete"/></button>
          </div>}
        ...
        {project && tab === "links" && <div aria-labelledby="manager-tab-links" className="manager-links manager-tab-panel" id="manager-panel-links" role="tabpanel">
          <ol>{links.map((link, linkIndex) => <li key={link.id}><button className="manager-link-open" onClick={() => void invoke("open_project_link", { linkId: link.id })} type="button"><span>{link.label}</span><small>{link.kind}</small></button><div className="manager-row-actions"><button aria-label={`Move ${link.label} up`} className="manager-icon-action" disabled={linkIndex === 0} onClick={() => void swapOrder("move_project_link", "linkId", links, linkIndex, -1)} title="Move up" type="button"><ManagerActionIcon name="up"/></button><button aria-label={`Move ${link.label} down`} className="manager-icon-action" disabled={linkIndex === links.length - 1} onClick={() => void swapOrder("move_project_link", "linkId", links, linkIndex, 1)} title="Move down" type="button"><ManagerActionIcon name="down"/></button><button aria-label={`Edit ${link.label}`} className="manager-icon-action" onClick={() => beginLinkEdit(link)} title="Edit" type="button"><ManagerActionIcon name="edit"/></button>{pendingRowDelete?.kind === "link" && pendingRowDelete.id === link.id ? <><button autoFocus className="is-danger" onClick={() => { void runMutation("delete_project_link", { linkId: link.id }); setPendingRowDelete(null); }} type="button">Confirm</button><button onClick={() => setPendingRowDelete(null)} type="button">Cancel</button></> : <button aria-label={`Delete ${link.label}`} className="manager-icon-action is-danger" onClick={() => setPendingRowDelete({ kind: "link", id: link.id })} title="Delete" type="button"><ManagerActionIcon name="delete"/></button>}</div></li>)}</ol>
