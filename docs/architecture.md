...
It is deliberately not built on to-dos: a to-do is decided once, while a dose is
a planned occurrence generated by a rule and is taken, skipped, or missed.

A treatment is a finite course with a name, notes, and a date range **derived**
from the earliest start and latest end of its medicines, so the two can never
disagree. A medicine carries display metadata (name, strength, form, dose
amount, food rule) and a schedule: local times of day plus a day pattern of
every day, every N days, or selected weekdays, within its own date range.

Planned doses are **materialized**, not derived on read. Every occurrence is a
stored row keyed by the civil `(medicineId, slotDay, slotTime)` triple, so the
past stays truthful when a schedule changes: a schedule edit regenerates only
occurrences that are both in the future and untouched, freezing anything taken,
skipped, notified, or already elapsed. Descriptive metadata edits regenerate
nothing, but they do relabel existing history, so a genuine regimen change is
expected to create a new medicine rather than edit one.

Storage, generation, identity, and ordering are civil, which is what makes
exactly one occurrence exist per local slot on daylight-saving transition days.
Elapsed-time questions — due versus missed, the reminder grace window, and the
schedule-edit cutoff — resolve the civil slot to a local instant instead:
ambiguous repeated times take the earlier occurrence, nonexistent skipped times
take the first valid instant after the gap, and future slots resolve in the
current timezone so a dose keeps its wall-clock time after travel.

Doses surface in the Today popup between meetings and to-dos, in an optional
widget destination segment whose badge and attention tone cover only the
current day, in an anchored panel bounded to eight dose rows across three
treatments, and in a Medicine window with Schedule, Today, Recent, and Notes.
Dose reminders are off by default and use the same poll-then-notify shape as
to-do reminders: the widget polls, Rust marks each dose notified once and
raises the toast. A dose is announced only inside its grace window, so one
missed while the app was closed is never announced late.
...
- WebView local storage: widget preferences, appearance, source order, calendar
  acknowledgement, to-do notification preference, medicine notification and
...
meeting sound, to-do notifications, and dose reminders are generated locally by
Attention Hub from its own calendar, reminder, and medicine state.

### Medicine data is local plaintext

`medicine.json` holds treatment names, medicine names, strengths, schedules,
and the complete dose history. It is stored **unencrypted**. Attention Hub adds
no password or encryption of its own, so the file is protected only by the
Windows user profile's access control and whatever full-disk encryption the
machine already has; anyone able to read the user's profile can read it.

This is disclosed in the product, not only here: the Medicine block on
Advanced / Reminders states it beside the storage path and the delete control.
Keeping medicine in its own file is what makes the exclusion from workspace
export a property of the storage layout rather than a filter that could be
forgotten.
